...
export default function EditorPage({ params }: { params: { id: string } }) {
    ...
    const handleImageLoad = (e: React.SyntheticEvent<HTMLImageElement>) => {
        ...
        const { naturalWidth, naturalHeight, src, currentSrc } = img;

        console.log('=== Image Load Debug ===');
        console.log('Original URL:', imageUrl);
        console.log('Actual loaded URL:', currentSrc || src);
        ...
        <div className="flex h-screen w-screen bg-neutral-900 text-white overflow-hidden fixed top-0 left-0">
            ...
            <main ref={mainRef} className="flex-1 w-0 relative bg-neutral-900/50 flex flex-col overflow-hidden z-10">
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2">
                    ...
                    {/* Debug Info */}
                    {imgSize.width > 0 && (
                        <div className="bg-neutral-800/90 backdrop-blur border border-neutral-700 px-3 py-1 rounded text-xs text-neutral-400">
                            원본: {imgSize.width} × {imgSize.height}px | 현재: {Math.round(imgSize.width * scale)} × {Math.round(imgSize.height * scale)}px
                        </div>
                    )}
                </div>
            ...
            {selectedItem ? (
                <aside className="w-80 bg-neutral-800 border-l border-neutral-700 flex flex-col z-40 flex-none shrink-0 transition-all">
                    <div className="p-4 border-b border-neutral-700 flex justify-between items-center">
                        <h3 className="font-semibold">텍스트 편집</h3>
                        <Button variant="ghost" size="icon" className="text-red-400 hover:text-red-300 hover:bg-red-950/30" onClick={() => deleteItem(selectedItem.id)}>
